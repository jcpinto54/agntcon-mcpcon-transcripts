---
title: "From Advisory to Autonomous: A Staged Model for Agent Adoption"
speakers: [Milos Mandic]
session_id: 22ac6e1508fb4cbe2c66b4d0c27bdc87
kind: materials
source: speaker
contributor: jcpinto54
---

# From Advisory to Autonomous: A Staged Model for Agent Adoption — materials

**Milos Mandic**

*Friday 18 September 2026, 15:35, G106 + G107 — Human-Agent Collab track*

> Material shared by the speaker. Not a transcript and not a summary. The deck
> was published by the organisers on the session's [Sched page](https://agntconmcpconeu26.sched.com/event/22ac6e1508fb4cbe2c66b4d0c27bdc87),
> and the extracted text sits beside it so the slides are searchable.

## Slides

- [From Advisory to Autonomous](materials/milos-mandic-from-advisory-to-autonomous.pdf) — 14 slides. Lays out a four-stage trust model (advisory, semi-autonomous, enhanced, full autonomy) for agent adoption, with gates and a sales-order case study. ([organisers' copy](https://hosted-files.sched.co/agntconmcpconeu26/52/Milos_Mandic_From_Advisory_to_Autonomous.pdf))
- Slide-by-slide description: [`materials/milos-mandic-from-advisory-to-autonomous.slides.md`](materials/milos-mandic-from-advisory-to-autonomous.slides.md)
