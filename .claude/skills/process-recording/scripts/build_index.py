...
transcript, so the gaps are visible and someone can fill them.
...
def main():
    ...
    # Map session_id -> transcript file, for whatever has been written so far.
    have = {}
    for path in sorted(glob.glob(os.path.join(ROOT, "talks", "*.md"))):
        fm = frontmatter(path)
        if fm.get("session_id"):
            have[fm["session_id"]] = os.path.basename(path)
    ...
    for day in ("thu", "fri"):
        ...
        lines.append(f"\n### {DAY_LABEL[day]}  ({done}/{len(day_sessions)} transcribed)\n")
        lines.append("| | Time | Room | Talk | Speakers |")
        lines.append("|---|---|---|---|---|")
        for s in day_sessions:
            fn = have.get(s["id"])
            mark = f"[x](talks/{fn})" if fn else " "
            title = s["title"].replace("|", "\\|")
            names = ", ".join(sp["n"] for sp in s["speakers"]).replace("|", "\\|")
            link = f"[{title}](talks/{fn})" if fn else title
            lines.append(f"| {mark} | {s['start']} | {s['room']} | {link} | {names} |")
